...
public abstract class RequestMessageProcessor<Q extends RequestMessage, S extends ResponseMessage>
    ...
    public void processMessage(ChannelHandlerContext channelHandlerContext, Q message) {
        logger.info("来自 data server[%s]的消息，类型是[%s]，内容为[%s]", message.getChannelId(), MsgTypeEnum.getMsgTypeEnumByCode(message.getMsgType()), message);
        ...
        logger.info("返回给 data server的消息，类型是[%s]，内容为[%s]", MsgTypeEnum.getMsgTypeEnumByCode(responseMessage.getMsgType()), responseMessage);
